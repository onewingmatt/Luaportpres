...
class Game:
    ...
    def pass_turn(self, player_id):
        ...
        if len(not_passed) <= 1:
            print(f"[PASS] All passed - pile won by player at idx {self.lead_player_idx}")
            lead_player = self.players.get(self.player_order[self.lead_player_idx]) if self.lead_player_idx < len(self.player_order) else None
            print(f"[PASS] Lead player: {lead_player.name if lead_player else 'UNKNOWN'}")

            # ✅ FIX: Reset current to lead player, then advance
            self.current_player_idx = self.lead_player_idx
            ...
            print(f"[PASS] Starting next meld with {lead_player.name if lead_player else 'UNKNOWN'}")
            self.next_player()
    ...
    def _assign_roles(self):
        ...
        print(f"[ROLES] Finished order: {[p.name for p in finished]}")
        for i, player in enumerate(finished):
            position = i + 1
            role = role_map.get(position, 'Citizen')
            player.role = role
            print(f"[ROLES] {player.name}: {role}")
    ...
    def _force_cpu_exchange(self, player):
        ...
        return self._execute_exchange_submission(player, cards)
